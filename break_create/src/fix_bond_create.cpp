...
void FixBondCreate::initial_integrate(int vflag)
{
...
void FixBondCreate::post_integrate()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // disallow bonding between atoms in the same inital molecule that
      // have the same atomtype
      // -- specific to a-BxC:Hy problem with B-B in same icosahedron

      if ((atom->molecule[i] == atom->molecule[j]) &&
          (atom->type[i] == atom->type[j])) continue;
...
//if (screen) fprintf(screen,"%s %i %i %i %i %i %i \n","CREATE i j iatomtype jatomtype inewtype jnewtype",i,j,iatomtype,jatomtype,inewtype,jnewtype);
      num_bond[i]++;
    }
  ...
  // trigger rebuilding of the bondcount array if bonds were formed

  bigint b_mod = atom->bonds_modified;
  if (createcount) b_mod = update->ntimestep;
...
/* ----------------------------------------------------------------------
   update bondcount array when needed
------------------------------------------------------------------------- */

void FixBondCreate::update_bondcount()
{
  int i,j,m;
    ...
    for (j = 0; j < atom->num_bond[i]; j++) {
      if (atom->bond_type[i][j] == btype) {
        ...
        if (force->newton_bond) {
          ...
          if (m < 0)
            error->one(FLERR,
                       "Could not update bondcount in fix bond/create");
          bondcount[m]++;
        }
      }
    }
